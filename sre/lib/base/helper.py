...
def create_iam_user(self):
  ...
  self.client.post('iam/users', data=json.dumps(payload), headers={"authorization": authorization(self), 'content-type': 'application/vnd.api+json'})
...
def create_cognito_user(self, identifier):
  ...
  return self.client.post('cognito/users', data=json.dumps(payload), headers={"authorization": authorization(self), 'content-type': 'application/vnd.api+json'} )

def login_cognito_user(self, identifier):
  payload = { "data": { "attributes": { "primary_identifier": identifier } } }
  return self.client.post('cognito/login', data=json.dumps(payload), headers={"authorization": authorization(self), 'content-type': 'application/vnd.api+json' } )
